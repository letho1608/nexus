...
import json
import os
from .models.llama import Transformer, TransformerShard, convert_from_huggingface, fix_bf16, sample_logits
...
import numpy as np
from .helpers import concat_weights, load
...
class TinygradDynamicShardInferenceEngine(InferenceEngine):
  ...
  async def infer_tensor(self, request_id: str, shard: Shard, input_data: np.ndarray, inference_state: Optional[dict] = None) -> tuple[np.ndarray, Optional[dict]]:
    ...
    def wrap_infer():
      ...
      out = self.model.forward(h, state["start_pos"], state["cache"])
