...
from topology import DeviceCapabilities, device_capabilities, UNKNOWN_DEVICE_CAPABILITIES
from utils.helpers import DEBUG, DEBUG_DISCOVERY, get_all_ip_addresses_and_interfaces, get_interface_priority_and_type
  ...
  async def task_broadcast_presence(self):
    while True:
      for addr, interface_name in get_all_ip_addresses_and_interfaces():
        # Skip APIPA (169.254.x.x) addresses as they don't support broadcast
        if addr.startswith("169.254."):
          if DEBUG_DISCOVERY >= 2: print(f"Skipping APIPA address: {addr}")
          continue
        ...
        try:
          ...
          if DEBUG_DISCOVERY >= 2: print(f"Error in broadcast presence ({addr} - {interface_name} - {interface_priority}): {e}")
        finally:
          if transport:
            try: transport.close()
            except Exception as e:
