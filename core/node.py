...
from inference.engine import InferenceEngine, Shard
from topology.topology import Topology
from topology import device_capabilities, UNKNOWN_DEVICE_CAPABILITIES
from topology import Partition, PartitioningStrategy, map_partitions_to_shards
from utils.helpers import DEBUG
from utils.helpers import AsyncCallbackSystem
from utils.topology_viz import TopologyViz
from download.manager import RepoProgressEvent
from inference.engine import get_inference_engine, InferenceEngine
from download.manager import ShardDownloader
  ...
  async def process_inference_result(
    ...
    if shard.model_id != 'stable-diffusion-2-1-base':
      ...
      if shard.is_last_layer() and not is_finished:
        ...
        # Check if tokenizer exists and has eos_token_id, otherwise use default
        eos_token_id = None
        if hasattr(self.inference_engine, 'tokenizer') and self.inference_engine.tokenizer and hasattr(self.inference_engine.tokenizer, 'eos_token_id'):
          eos_token_id = self.inference_engine.tokenizer.eos_token_id
        is_finished = (eos_token_id is not None and token.item() == eos_token_id) or is_finished or len(self.buffered_token_output[request_id][0]) >= self.max_generate_tokens
